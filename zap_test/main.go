...
func main() {
	InitLog("./zap_test/info.log", "./zap_test/error.log", zap.InfoLevel)
	defer logger.Sync()

	sugarLogger.Infof("sugarLogger name:%s", "test1")
	sugarLogger.Infow("sugarLogger", zap.String("name", "test2"))
	sugarLogger.Errorf("sugarLogger name:%s", "test3")
	sugarLogger.Debugf("sugarLogger name:%s", "test4")
	sugarLogger.Warnf("sugarLogger name:%s", "test5")

	logger.Info("logger", zap.String("name", "test6"))
	logger.Error("logger", zap.String("name", "test7"))
	logger.Debug("logger", zap.String("name", "test8"))
}
...
func InitLog(logPath, errPath string, logLevel zapcore.Level) {
	...
	logger = zap.New(core, zap.AddCaller(), zap.AddStacktrace(zap.WarnLevel), zap.Hooks(func(entry zapcore.Entry) error {
